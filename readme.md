...
- replace text `Babita_Business_Theme` to `acme_block_theme`
...
2. Open `.bin/initial-rename.js` and replace the `from:` and `to:`
   ...
   - `Babita_Business_Theme`  → `[Your_Theme_PascalCase]`
   - `Babita_Business_Theme`  → `wp_[your_slug_snake]`
   - `Babita_Business_Theme`  → `[YOUR_SLUG_UPPER]`
