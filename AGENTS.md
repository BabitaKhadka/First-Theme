...
- **Version constant:** `BABITA_BUSINESS_THEME_VERSION` (in `functions.php`)
...
3. **Edit `.bin/initial-rename.js`** — define new identifiers:
   ...
   - `Babita_Business_Theme` → `<New_Slug_PascalCase>`
   - `Babita_Business_Theme` → `wp_<new_slug_snake>`
   - `Babita_Business_Theme` → `<NEW_SLUG_UPPER>`
...
```
includes/main.php             Core class Babita_Business_Theme, loads deps
...
- **Version:** `BABITA_BUSINESS_THEME_VERSION` constant. Use this in
...
1. Subclass `Babita_Business_Theme_Api` in `includes/api/`.
...
|---|---|---|
| `Babita_Business_Theme_parse_changelog()` | Sanitized text of the `== Changelog ==` section of `readme.txt`, including per-version headings (e.g. `= 1.2.3 =`), bullets, and blank lines between versions. Empty string when the section is missing. | `admin/templates/page-theme-info.php` Changelog card |
| `Babita_Business_Theme_get_theme_faq()` | Array of `{q, a}` pairs rendered in the FAQ accordion. | `admin/templates/page-theme-info.php` FAQ card |
| `Babita_Business_Theme_get_recommended_plugins()` | Array of plugin info entries (`name`, `slug`, `plugin`, optional `source`/`url`). | Admin Recommended Plugins card + REST `/install-plugin` endpoint |
| `Babita_Business_Theme_install_plugin()` | Result array `{success, message}` after installing/activating a plugin. | `includes/api/class-api-install-plugin.php` |
| `Babita_Business_Theme_file_system()` | Initialized `WP_Filesystem` instance. | `Babita_Business_Theme_parse_changelog()` |

### Filters

- `Babita_Business_Theme_changelog_file` — override the absolute path
  to `readme.txt` for `Babita_Business_Theme_parse_changelog()`. Useful
  for tests or for themes whose changelog lives in a separate file.
- `Babita_Business_Theme_recommended_plugins` — add or remove
  entries from the Recommended Plugins list.
- `Babita_Business_Theme_default_options`,
  `Babita_Business_Theme_default_user_meta` — extend the theme
  options / user-meta defaults.
- `Babita_Business_Theme_faq` — add or remove FAQ entries.
