 ...
 * @subpackage Babita_Business_Theme/Babita_Business_Theme_Info
 */
 ...
 */
class Babita_Business_Theme_Info {
	...
	public function add_theme_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Theme Info', 'wp-block-theme-boilerplate' ), esc_html__( 'Theme Info', 'wp-block-theme-boilerplate' ), 'edit_theme_options', BABITA_BUSINESS_THEME_NAME, array( $this, 'info_screen' ) );
	}
	...
	public function enqueue_resources( $hook_suffix ) {
		...
		$unique_id = BABITA_BUSINESS_THEME_NAME . '-info';
		...
		$deps_file = BABITA_BUSINESS_THEME_PATH . 'build/admin/info/info.asset.php';
		...
		wp_enqueue_script( $unique_id, BABITA_BUSINESS_THEME_URL . 'build/admin/info/info.js', $dependency, $version, true );

		wp_enqueue_style( $unique_id, BABITA_BUSINESS_THEME_URL . 'build/admin/info/info.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'Babita_Business_Theme_info_localize',
			array(
				'version'  => $version,
...
/**
 * Return instance of Babita_Business_Theme_Info class
 *
 * @since 1.0.0
 *
 * @return Babita_Business_Theme_Info
 */
function Babita_Business_Theme_info() { //phpcs:ignore
	return Babita_Business_Theme_Info::get_instance();
}
Babita_Business_Theme_info()->run();
