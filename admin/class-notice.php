 ...
 */
class Babita_Business_Theme_Notice {
	...
	private function is_load_getting_started() {
		return ! Babita_Business_Theme_include()->get_settings( 'hide_get_started_notice' );
	}
	...
	private function is_load_review() {
		...
		$remove_review_notice_permanently         = Babita_Business_Theme_include()->get_user_meta( $user_id, 'remove_review_notice_permanently' );
		$remove_review_notice_temporary_date_time = Babita_Business_Theme_include()->get_user_meta( $user_id, 'remove_review_notice_temporary_date_time' );

		$theme_installed_date_time = Babita_Business_Theme_include()->get_settings( 'theme_installed_date_time' );
	...
	public function enqueue_resources() {
		...
		$unique_id = BABITA_BUSINESS_THEME_NAME . '-notice';
		...
		$deps_file = BABITA_BUSINESS_THEME_PATH . 'build/admin/notice/notice.asset.php';
		...
		wp_enqueue_script( $unique_id, BABITA_BUSINESS_THEME_URL . 'build/admin/notice/notice.js', $dependency, $version, true );
		wp_enqueue_style( $unique_id, BABITA_BUSINESS_THEME_URL . 'build/admin/notice/notice.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'Babita_Business_Theme_notice_localize',
			array(
				'version'             => $version,
				'nonce'               => wp_create_nonce( 'wp_rest' ),
				'rest_url'            => get_rest_url(),
				'recommended_plugins' => Babita_Business_Theme_get_recommended_plugins(),
				'theme_info_url'      => esc_url( menu_page_url( BABITA_BUSINESS_THEME_NAME, false ) ),
			)
		);
	...
	public function add_theme_data() {
		...
		if ( ! $theme_installed_date_time ) {
			Babita_Business_Theme_update_options( 'theme_installed_date_time', time() );
		}
	}
	...
	public function remove_theme_data() {
		...
		delete_option( BABITA_BUSINESS_THEME_OPTION_NAME );

		// Delete the user meta for all users.
		delete_metadata(
			'user',
			0,
			BABITA_BUSINESS_THEME_OPTION_NAME,
			'',
			true
		);
	}
...
/**
 * Return instance of  Babita_Business_Theme_Notice class
 *
 * @since 1.0.0
 *
 * @return Babita_Business_Theme_Notice
 */
function Babita_Business_Theme_notice() { //phpcs:ignore
	return Babita_Business_Theme_Notice::get_instance();
}
Babita_Business_Theme_notice()->run();
